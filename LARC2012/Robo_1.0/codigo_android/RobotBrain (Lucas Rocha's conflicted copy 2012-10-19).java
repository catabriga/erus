...
public class RobotBrain
{
	...
	private int turnToDeegre;	

	private int state, turnState;
	private double degrees, initialDegrees;
	...
	private void checkObstacle(UltraSound ult) throws IOException
	{
		if(ult.getUs6() < 40)
		{
	...
	{			
		...
		//else if(can.minY < CATCHABLE_CAN_LIMIT_Y_MIN)
		{
			if(System.currentTimeMillis() > time + 100)
			{
				...
				setMotorsMovement( -100 + error, -100 - error);					
			}
		}
		else
		{			
	...
	private void stateSearchTrash(Accelerometer acc, Compass comp, Encoder enc, UltraSound ult, ClawButton cb, CameraProcessor cameraProcessor)
	{
		if(System.currentTimeMillis()%100 > 50)
		{
	...
	private void stateSearchTrashLeft1(Accelerometer acc, Compass comp, Encoder enc, UltraSound ult, ClawButton cb, CameraProcessor cameraProcessor) throws IOException
	{
		...
		if(cameraProcessor.getTrashPosition().x >= 0)
		{
			state = GO_TO_TRASH;
		}
		else
...
/*		else if( can.minY < CATCHABLE_CAN_LIMIT_Y_MIN)
	...
	{
		//USAR FUNÇÃO DE TURN: turn -> 180º
		if(lastState != MANOUVER_TO_TRASH)
		{
			time = System.currentTimeMillis() + 1000;
		}
	...
	public void process(Accelerometer acc, Compass comp, Encoder enc, UltraSound ult, ClawButton cb, CameraProcessor cameraProcessor)
	{
